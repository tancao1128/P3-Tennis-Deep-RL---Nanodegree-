...
_eps_start = 1.0         # initial value for epsilon in noise decay process in Agent.act()
...
class Agent():
    ...
    def __init__(self, state_size, action_size, num_agents, random_seed):
        ...
        self.eps = _eps_start
        self.eps_decay = 1/(_eps_ep_end*_learn_num)  # set decay rate based on epsilon end target
        self.timestep = 0
        ...
        self.critic_optimizer = optim.Adam(self.critic_local.parameters(), lr=_lr_critic, weight_decay=_weight_decay)
    ...
    def step(self, state, action, reward, next_state, done, agent_number):
        ...
        if len(self.memory) > _batch_size and self.timestep % _learn_every == 0:
                for _ in range(_learn_num):
                    experiences = self.memory.sample()
                    self.learn(experiences, _gamma, agent_number)
    ...
    def learn(self, experiences, gamma, agent_number):
        ...
        self.eps = max(self.eps, _eps_final)
        self.noise.reset()
...
class OUNoise:
    ...
    def __init__(self, size, seed, mu=0.0, theta=_ou_theta, sigma=_ou_sigma):
